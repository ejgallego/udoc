...
module Html : Output.S = struct
  ...
  let header () =
    if !opts.header_trailer then
    if !opts.header_file_spec then
      let cin = Pervasives.open_in !opts.header_file in
      try
        while true do
  ...
  let trailer () =
    if !opts.header_trailer && !opts.footer_file_spec then
	let cin = Pervasives.open_in !opts.footer_file in
	  try
	    while true do
	      let s = Pervasives.input_line cin in
		printf "%s\n" s
      ...
      begin
        if !opts.index && !cur_mod <> "Index" then
          printf "</div>\n\n<div id=\"footer\">\n<hr/><a href=\"%s.html\">Index</a>" !opts.index_name;
  ...
  let start_module coq_mod =
    ...
    printf "<h1 class=\"libtitle\">Module %s</h1>\n\n" mod_name
  ...
  module Appendix = struct
    ...
    let make_toc out =
	  ...
	  let ms = match sub with | None -> m | Some s -> m ^ ": " ^ s in
 	    printf "<a href=\"%s.html\"><h2>Module %s</h2></a>\n" m ms
        | Toc_section (n, f, r) ->
  	  item n;
  	  printf "<a href=\"%s\">" r; f (); printf "</a>\n"
      in
      page_title := (if !opts.title <> "" then !opts.title else "Table of contents");
      header ();
      if !opts.title <> "" then printf "<h1>%s</h1>\n" !opts.title;
      printf "<div id=\"toc\">\n";
    ...
    let index_ref i c =
      ...
      !opts.index_name ^ (if !opts.multi_index then "_" ^ idxc ^ ".html" else ".html#" ^ idxc)
    ...
    let make_one_multi_index prt_tbl i =
      ...
      let one_letter ((c,l) as cl) =
        with_outfile (sprintf "%s_%s_%c.html" !opts.index_name idx c) (fun fmt ->
            oc := fmt;
            if (!opts.header_trailer) then header ();
            prt_tbl (); printf "<hr/>";
            letter_index true idx cl;
            if List.length l > 30 then begin printf "<hr/>"; prt_tbl () end;
            if (!opts.header_trailer) then trailer ()
          )
      in
  ...
  let appendix ~toc ~index ~split_index ~standalone =
    ...
    if index       then with_outfile (!opts.index_name^".html") Appendix.make_index;
